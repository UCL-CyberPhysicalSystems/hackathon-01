...
def frames_listener(sample):
    npImage = np.frombuffer(bytes(sample.payload), dtype=np.uint8) # Or convert to bytes first
    #npImage = np.frombuffer(sample.payload.contiguous_data(), dtype=np.uint8) # Access the payload directly
    #npImage = np.frombuffer(bytes(sample.value.payload), dtype=np.uint8) #original
